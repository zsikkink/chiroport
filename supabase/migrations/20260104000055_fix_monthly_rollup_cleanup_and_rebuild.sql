...
with months as (
  ...
  where local_date < (current_date - interval '7 days')::date
  group by location_id, month_start
)
delete from public.queue_monthly_stats m
using months mm
where m.location_id = mm.location_id
  and m.month_start = mm.month_start;
...
delete from public.queue_monthly_stats m
using global_months gm
where m.location_id is null
  and m.month_start = gm.month_start;

select public.rollup_queue_monthly_stats();
