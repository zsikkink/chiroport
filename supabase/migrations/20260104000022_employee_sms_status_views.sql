...
-- Allow employees to read sms_outbox for dashboard status indicators.
drop policy if exists "Employees read sms outbox" on public.sms_outbox;
create policy "Employees read sms outbox"
...
-- Refresh employee-facing views with sms status columns.
