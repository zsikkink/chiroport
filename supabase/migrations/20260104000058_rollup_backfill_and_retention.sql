...
create table if not exists public.queue_rollup_settings (
  id boolean primary key default true,
  daily_stats_retention_days integer not null default 90,
  monthly_rollup_lag_days integer not null default 7,
  archive_retention_days integer not null default 365,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint queue_rollup_settings_single_row check (id = true),
  constraint queue_rollup_settings_daily_retention_positive
    check (daily_stats_retention_days > 0),
  constraint queue_rollup_settings_monthly_lag_positive
    check (monthly_rollup_lag_days > 0),
  constraint queue_rollup_settings_archive_retention_positive
    check (archive_retention_days > 0)
);

insert into public.queue_rollup_settings (id)
values (true)
on conflict (id) do nothing;

create table if not exists public.queue_entries_archive (
  id uuid primary key,
  ...
  service_label text not null default 'Unknown',
...
create index if not exists idx_queue_entries_archive_location_date
  on public.queue_entries_archive (location_id, local_date);
create index if not exists idx_queue_entries_archive_local_date
  on public.queue_entries_archive (local_date);
create index if not exists idx_queue_entries_archive_created_at
  on public.queue_entries_archive (created_at);

alter table public.queue_entries_archive enable row level security;
...
create or replace view public.queue_entries_history as
select
  qe.id,
  qe.queue_id,
  q.location_id,
...
select
  ...
  qe.cancelled_at,
  qe.no_show_at
from public.queue_entries_archive qe;
...
begin
  ...
  select
    ...
    e.customer_type,
    e.status,
    coalesce(e.service_label, 'Unknown') as service_label,
    e.created_at,
    e.updated_at,
    e.served_at,
    e.completed_at,
...
create or replace function public.backfill_queue_daily_stats(
  p_start date,
  p_end date
)
returns void
...
begin
  if p_start is null or p_end is null or p_end < p_start then
    raise exception 'Invalid date range for backfill.';
  end if;

  delete from public.queue_daily_stats d
  where d.local_date between p_start and p_end;

  with entries as (
    select
      e.*,
      coalesce(e.service_label, 'Unknown') as service_label_group
    from public.queue_entries_history e
...
create or replace function public.rebuild_queue_monthly_stats(
  p_start date,
  p_end date
)
returns void
...
begin
  if p_start is null or p_end is null or p_end < p_start then
    raise exception 'Invalid date range for rebuild.';
  end if;

  create temporary table tmp_rebuild_months on commit drop as
  select distinct date_trunc('month', local_date)::date as month_start
  from public.queue_daily_stats
  where local_date between p_start and p_end;
  ...
  with daily_rows as (
    select
      ...
      date_trunc('month', d.local_date)::date as month_start
    from public.queue_daily_stats d
    join tmp_rebuild_months m
      on m.month_start = date_trunc('month', d.local_date)::date
  )
  select
    location_id,
    month_start,
...
create or replace function public.backfill_queue_rollups(
  p_start date,
  p_end date
)
returns void
...
as $$
begin
  perform public.backfill_queue_daily_stats(p_start, p_end);
  perform public.rebuild_queue_monthly_stats(p_start, p_end);
end;
$$;
