...
set service_label = case
  when customer_type = 'priority_pass' then 'Priority Pass'
  else 'Paying'
end
where service_label is null;

alter table public.queue_entries
  alter column service_label set default 'Paying',
  alter column service_label set not null;

-- Update join_queue to accept and persist service labels.
...
-- Add service labels to employee-facing views.
