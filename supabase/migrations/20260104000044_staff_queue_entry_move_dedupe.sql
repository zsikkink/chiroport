...
begin
  ...
  v_result_queue_id := v_entry.queue_id;
  ...
  if p_action = 'serving' then
    ...
    with cancelled as (
      ...
      set status = 'cancelled',
          cancelled_at = now()
      where qe.queue_id = v_target_queue_id
        and qe.customer_id = v_entry.customer_id
    ...
    select
      ...
      p_actor_user_id,
      'cancelled_by_move',
      jsonb_build_object('target_queue_id', v_target_queue_id)
    from cancelled;

    v_next_sort_key := public.next_sort_key(v_target_queue_id, v_entry.customer_type);

    v_new_status := 'waiting';
    v_result_queue_id := v_target_queue_id;
    update public.queue_entries
  ...
  out_queue_id := v_result_queue_id;
